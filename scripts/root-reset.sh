...
btrfs subvolume show ${BLANK_ROOT_SNAPSHOT} &>/dev/null && SNAPSHOT_EXISTS=true
if [ ! $SNAPSHOT_EXISTS ];
then
	echo "Snapshot '$BLANK_ROOT_SNAPSHOT' not found. Creating it now."
	btrfs subvolume snapshot -r "${MOUNTDIR}/root" "${MOUNTDIR}/root-blank"
	LEAVE_MNT_FOR_INSTALL=true
else
	LEAVE_MNT_FOR_INSTALL=false
fi
