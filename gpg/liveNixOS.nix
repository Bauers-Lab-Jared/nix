let
  configuration = { config, lib, pkgs, ... }:
    with pkgs;
    let
      guidesrc = fetchGit "https://github.com/drduh/YubiKey-Guide";

      guide = "${guidesrc}/README.md";

      waffleConfig = fetchGit "https://github.com/Bauers-Lab-Jared/flakes.git";

      gpg-conf = "${waffleConfig}/gpg/gpg.conf";
      ...
      gpg-agent-conf = runCommand "gpg-agent.conf" {} ''
        sed '/pinentry-program/d' ${waffleConfig}/gpg/gpg-agent.conf > $out
